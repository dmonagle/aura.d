...
private:
	void runQueue() {
		scope(exit) {
			logInfo("Stopping %s(%s) queue".color(fg.light_red), _name, T.stringof);
		}
		logInfo("Starting %s(%s) queue".color(fg.light_green), _name, T.stringof);
